...
export const CreateTaskButtons = () => {
  ...
  function createTask() {
    const task = { alg: "Pyro", semicolon: ",", error: 0.01 };

    const request = new XMLHttpRequest();

    request.open("POST", "http://localhost:5000/createTask", true);
    request.setRequestHeader("Content-Type", "application/json");
    request.addEventListener("load", () => {
      const answer = JSON.parse(request.response);
      setStatus(answer.status);
      setTaskID(answer.taskID);
    });
  ...
  function createTask2() {
    const requestOptions = {
      ...
      body: JSON.stringify({ alg: "Tane", semicolon: ",", error: 0.02 }),
    };
  ...
  return (
    <div>
      ...
      <Button onClick={createTask2}>Create new task</Button>
      <div style={{
        margin: "15px",
        padding: "10px",
        width: " 400px",
