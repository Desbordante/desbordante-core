...
is built with Clang, which has a different ABI. Instead, download the latest version of Boost from the [official website](https://www.boost.org/users/download/) and unpack the archive to the `/usr/local/lib` directory or another directory of your choice:
```sh
cd /usr/local/lib
...
export BOOST_COMPILER=gcc14
``` 
You can also add last 2 export with fixed Boost root path to `~/.zprofile` or `~/.bash_profile` to set them by default.
...
```
The first two lines set GCC as default compiler in CMake. The last export is also necessary due to issues with GCC 14 and
