...
['Yogurt'] -> ['Eggs'] with confidence 0.67 means that when milk is found in the receipt, the chance of eggs being present amounts to 67%. So, customers are \x1b[1;33mlikely\x1b[0m to buy eggs with milk.
...
Now you can see that the number of association rules have decreased significantly. This happened due to minsup being set to 0.5. 
...
A fragment of the table is presented above. In total, each dog breed has 13 attributes.
...
Training Difficulty[4 - 8]
Friendliness[5 - 8]
   support = 0.2893081761006289
...
snapshots['test_example[basic/verifying_dc.py-None-verifying_dcoutput] verifying_dcoutput'] = '''This is an example of how to use Denial Constraint (DC) verification for checking hypotheses on data.
...
And the following Denial Constraint: !(s.State == t.State and s.Salary < t.Salary and s.FedTaxRate > t.FedTaxRate).
...
Now we will modify the initial dataset to make things a little bit more interesting.
