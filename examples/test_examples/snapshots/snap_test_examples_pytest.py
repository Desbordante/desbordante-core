...
Furthermore, AC mining algorithm provides five parameters for setting up execution:
weight
fuzziness
p_fuzz
bumps_limit
...
Weight accepts values in the range (0, 1]. 
...
Fuzziness belongs to (0,1) range while p_fuzz belongs to [0,1] range. These parameters 
...
Bumps_limit accepts only natural numbers from the range [1, inf) and limits the number of 
intervals for all column pairs. To set bumps_limit to inf you should use the 0 value.

Iterations_limit accepts only natural numbers. 
Lower values (close to 1) reduce accuracy due to algorithm performing fewer iterations.

AC_seed accepts only natural numbers. 
...
binaty operation is "-", weight - 0.1, fuzziness - {FUZZINESS}, p_fuzz - {P_FUZZ}, bumps_limit - {BUMPS_LIMIT}, 
...
However, with this amount of intervals, it is difficult to draw immediate conclusions about the 
...
Another option is to try to find a parameter combination that will result in a fewer number of 
