...
of two segments:
  * temperature: [85, 95];
  * RPM: [1500, 3500].
This forms a two-dimensional parallelepiped domain: {BLUE}[85, 95] x [1500, 3500]{ENDC}.
"""
)
# Arguments of generic version are A = (a1, a2, ..., an) and B = (b1, b2, ..., bn).
# Domain is [A, B] = [a1, b1] x [a2, b2] x ... x [an, bn].
parallelepiped = desbordante.pac.domains.Parallelepiped(["85", "1500"], ["95", "3500"])
...
algo.load_data(
    table=(ENGINE_TEMPS, ",", True), domain=parallelepiped, column_indices=[0, 1]
)
...
parallelepiped = desbordante.pac.domains.Parallelepiped(
    ["85", "1500"], ["95", "3500"], [1, 0.01]
)
algo = desbordante.pac_verification.algorithms.DomainPACVerifier()
algo.load_data(
    table=(ENGINE_TEMPS, ",", True), domain=parallelepiped, column_indices=[0, 1]
