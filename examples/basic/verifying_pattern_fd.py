...
def run_example():
    table = pandas.read_csv(TABLE_PATH, header=0)
    ...
    print("We want each pattern to appear at least 4 times, with no more than 25% of them matching by City but differing by Licensee Number. " \
          "Also, we want the entire functional dependency to cover at least 10 rows.\n")

    algo.execute(lhs_indices=lhs, rhs_index=rhs, 
...
if __name__ == '__main__':
    print("This example demonstrates how to validate Pattern Functional Dependencies (Pattern FDs) using the Desbordante library.\n"
        ...
        "  • min_pattern_inclusion: Minimum records a single pattern must cover\n"
        "  • min_pattern_fd_coverage: Minimum total records covered by the entire Pattern FD\n"
        "  • max_rhs_deviation: Maximum allowed violation rate (mismatched Y values for matching X patterns)\n"
        "Desbordante detects Pattern FD violations and estimates real parameter values.\n")
