...
def print_intro():
    ...
    printlns(
        ...
        "dependencies (CFDs) in data using the CFUN algorithm from the paper [1]. "
...
def print_dataset_description(df):
    ...
    print()
    print("Dataset:")
    print(tabulate(df, headers="keys", tablefmt="psql", showindex=range(1, len(df) + 1)))
    print()
...
def print_summary():
    banner("Summary")
    printlns(
        "You have now mastered the core idea behind constant CFDs and know how to apply it in practice. " \
        "In real data, use CFUN iteratively: try several support thresholds, inspect the "
