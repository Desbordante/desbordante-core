...
def mine_sfd():
    ...
    algo.execute(
        min_sfd_strength=MIN_SFD_STRENGTH_MEASURE,
        delta=DELTA,
...
if __name__ == "__main__":
    print(
        "A soft functional dependency (SFD) is yet another type of relaxed functional dependency (FD),\n"
    ...
    print(f"First of all let us construct a synthetic dataset consisting of {n_rows} rows and 5 columns.\n")
    ...
    D = 3 * C + np.random.choice([0, 5, 17, 34], n_rows, p=[0.6, 0.1, 0.1, 0.2])
    E = np.random.randint(1, 3, n_rows)  # Independent column
    # Create a DataFrame
    df = pd.DataFrame({"A": A, "B": B, "C": C, "D": D, "E": E})
    print("Our dataset contains 5 columns: A, B, C, D, E")
    print(
        "A is a column of random integers belonging to [1,100).\n"
