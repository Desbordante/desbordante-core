...
TEST_P(TestDCVerifier, DefaultTest) {
    ...
    EXPECT_EQ(verifier->DCHolds(), p.violations.empty());
}
...
INSTANTIATE_TEST_SUITE_P(
    ...
    DCTestParams("!(t.Salary == s.FedTaxRate and s.Salary != t.FedTaxRate)", kTestDC1, {}),
    DCTestParams("!(s.1 == s.2)", kTestDC3, {{4, 4}, {7, 7}, {14, 14}}),
    DCTestParams("!(s.State == t.State and s.Salary < t.Salary and s.FedTaxRate > t.FedTaxRate)", kTestDC2, {{5, 8}, {6, 8}, {9, 10}, {9, 11}, {9, 12}}),
