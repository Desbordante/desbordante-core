...
public:
    ...
    unsigned long long GetVal1() const {
        unsigned long long val1 = std::pow(2, T::Size - 1) + 1;
        auto val2 = std::numeric_limits<unsigned long long>::max() - 2;
        return std::max(val1, val2);
    }

    unsigned long long GetVal2() const {
        unsigned long long val1 = std::pow(2, T::Size - 1) - 2;
        auto val2 = std::numeric_limits<unsigned long long>::max() - 1;
        return std::max(val1, val2);
    }
};
...
TYPED_TEST_P(BitsetTest, ULLongConstructor) {
    typename TypeParam::Custom c{this->GetVal1()};
    typename TypeParam::STL s{this->GetVal1()};

    // Check that bitsets are created properly:
    LOG(INFO) << "Bitsets with value " << this->GetVal1() << ':';
...
TYPED_TEST_P(BitsetTest, OperatorsEqNeq) {
    typename TypeParam::Custom c1{this->GetVal1()};
    typename TypeParam::Custom c2{this->GetVal1()};
    typename TypeParam::Custom c3{this->GetVal2()};
...
TYPED_TEST_P(BitsetTest, TestInvalidIndex) {
    ...
    EXPECT_THROW(c.test(TypeParam::Size), std::out_of_range);
}

TYPED_TEST_P(BitsetTest, All) {
    std::string str1(TypeParam::Size, '1');
...
TYPED_TEST_P(BitsetTest, Any) {
    std::string str1(TypeParam::Size, '0');
    std::string str2{str1};
    str2.back() = '1';
...
TYPED_TEST_P(BitsetTest, AndAssign) {
    typename TypeParam::Custom c1{this->GetVal1()};
    typename TypeParam::Custom c2{this->GetVal2()};
    typename TypeParam::STL s1{this->GetVal1()};
    typename TypeParam::STL s2{this->GetVal2()};
...
TYPED_TEST_P(BitsetTest, SetInvalidIndex) {
    ...
    EXPECT_THROW(c.set(TypeParam::Size), std::out_of_range);
}
...
TYPED_TEST_P(BitsetTest, ResetInvalidIndex) {
    ...
    EXPECT_THROW(c.reset(TypeParam::Size), std::out_of_range);
}
...
TYPED_TEST_P(BitsetTest, FlipInvalidIndex) {
    ...
    EXPECT_THROW(c.flip(TypeParam::Size), std::out_of_range);
}
...
REGISTER_TYPED_TEST_SUITE_P(BitsetTest, DefaultConstructor, ULongConstructor, ULLongConstructor,
                            ...
                            All, Any, None, Count, Size, AndAssign, OrAssign, XorAssign,
