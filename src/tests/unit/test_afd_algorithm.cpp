...
testing::AssertionResult CheckAfdListEquality(
    ...
    for (auto& afd : expected) {
        std::vector<unsigned int> lhs_indices = BitsetToIndexVector(afd.GetLhs().GetColumnIndices());
...
void MaxLhsTestFun(CSVConfig config, std::list<AFD> const& afds_list, config::MaxLhsType max_lhs) {
    ...
    auto verify_algo = algos::CreateAndLoadAlgorithm<algos::Tane>(verify_params);
    verify_algo->Execute();
    auto verify_list = AFDsToSet(verify_algo->AfdList());
...
TYPED_TEST_P(AlgorithmAFDTest, GettingThreeshold) {
    auto algorithm = TestFixture::CreateAlgorithmInstance(kTestLong);
    algorithm->Execute();
    std::list<AFD> afds = algorithm->AfdList();
    
    for (AFD afd : afds) {
        std::cout << afd.ToLongString() << ": ";
        std::cout << afd.GetThreeshold() << std::endl;
    }
}

REGISTER_TYPED_TEST_SUITE_P(AlgorithmAFDTest, GettingThreeshold);

using AFDAlgorithms =
        ::testing::Types<algos::Tane, algos::PFDTane>;
