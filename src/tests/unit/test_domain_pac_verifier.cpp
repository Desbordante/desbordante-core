...
struct DomainPACVerifyingParams {
    ...
    DomainPACVerifyingParams(CSVConfig const& csv_config, config::IndicesType&& col_indices,
                             ...
                             double expected_delta, std::optional<double> min_delta = std::nullopt,
        ...
        : params({
                  ...
                  {kMinEpsilon, OptToAny(std::move(min_eps))},
                  {kMaxEpsilon, OptToAny(std::move(max_eps))},
          }),
...
DomainPACVerifyingParams CustomMetricBallsIntervalsParams(
        double min_eps, double max_eps, double expected_eps, double expected_delta,
...
INSTANTIATE_TEST_SUITE_P(
        ...
        ::testing::ValuesIn({
                ...
                CustomMetricBallsIntervalsParams(0, -1, 6.217, 0.999, 0.95),
                ...
                CustomMetricBallsIntervalsParams(0, 1.3, 1.074, 0.705, 0.68),
                // -- Validation --
                // #20
                // Epsilon validation
                ...
                SimpleTyposParallelepipedIntervalsParams(0, 5, 0, 2, 0, 0.9),
                SimpleTyposParallelepipedIntervalsParams(0, 5, 2, 2, 0, 0.9),
                // Empty domain
                SimpleTyposParallelepipedIntervalsParams(7, 7, 0, 2, 0, 0),
        }));
...
INSTANTIATE_TEST_SUITE_P(
        ...
        ::testing::ValuesIn({
                // Some values fall into domain, highlighted values on both sides of domain
