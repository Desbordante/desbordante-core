...
struct FDVerifyingParams {
    ...
    FDVerifyingParams(
            std::pair<std::initializer_list<model::Index>, std::initializer_list<model::Index>>
                    fd_input_pair,
        ...
        : params({{onam::kCsvConfig, csv_config},
                  {onam::kFd,
                   model::FdInput({fd_input_pair.first.begin(), fd_input_pair.first.end()},
                                  {fd_input_pair.second.begin(), fd_input_pair.second.end()})},
...
INSTANTIATE_TEST_SUITE_P(
        ...
        ::testing::Values(
                FDVerifyingParams({{1}, {0}}), FDVerifyingParams({{2}, {0}}),
                FDVerifyingParams({{2}, {1}}), FDVerifyingParams({{0, 1, 2, 3, 4}, {5}}),
                FDVerifyingParams({{2, 3}, {5}}), FDVerifyingParams({{5}, {0}}),
                FDVerifyingParams({{5}, {1}}), FDVerifyingParams({{5}, {2}}),
                FDVerifyingParams({{5}, {3}}), FDVerifyingParams({{5}, {4}}),
                FDVerifyingParams({{1, 3}, {4}}), FDVerifyingParams({{5}, {0, 1, 2, 3, 4}}),
                FDVerifyingParams({{2}, {0, 1}}), FDVerifyingParams({{2, 3}, {0, 1, 4, 5}}),
                FDVerifyingParams({{2, 4}, {0, 1, 3, 5}}), FDVerifyingParams({{3, 4}, {0, 1}}),
                FDVerifyingParams({{1, 4}, {0, 3}}), FDVerifyingParams({{1, 3}, {0, 3}}),
