...
INSTANTIATE_TEST_SUITE_P(NDVerifierTestSuite, TestNDVerifying,
                         ::testing::Values(
                                 // Constant lhs:
                                 NDVerifyingParams({0}, {1}, 4), NDVerifyingParams({0}, {2}, 6),
                                 NDVerifyingParams({0}, {3}, 4), NDVerifyingParams({0}, {4}, 5),
                                 NDVerifyingParams({0}, {5}, 9),
                                 // Non-constant lhs:
                                 NDVerifyingParams({1}, {5}, 3), NDVerifyingParams({4}, {0}, 1),
                                 // Multiple columns:
...
INSTANTIATE_TEST_SUITE_P(
        ...
        ::testing::Values(
                NDVerifyingParams({5}, {6}, 1000000,
                                  kIowa1kk),  // I just want to see execution time. Real weight
                                              // doesn't matter (but it shouldn't be very big)
                NDVerifyingParams({16, 17, 18}, {20, 23}, 1000000,
                                  kIowa1kk)  // Also, I want to see how execution time depends on
                                             // number of columns
                ));

INSTANTIATE_TEST_SUITE_P(NDVerifierTestNullEqualNull, TestNDVerifying,
