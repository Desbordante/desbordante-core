...
public:
    ...
    /// @note Some checks are performed, but don't expect too much.
    static void Save(std::unordered_map<std::string, BenchmarkResult> const& results,
        ...
        for (auto const& [name, bm_res] : results) {
            boost::json::object bm_obj(
                    {{"name", name}, {"time", bm_res.result}, {"threshold", bm_res.threshold}});
