...
void BindFd(py::module_& main_module) {
    ...
    py::class_<FD>(fd_module, "FD")
            ...
            .def(py::pickle(
                // __getstate__
                [](FD const& fd) {
                    auto schema_state = table_serialization::SerializeRelationalSchema(fd.GetSchema());
                    auto lhs_state = table_serialization::SerializeVertical(fd.GetLhs());
                    auto rhs_state = table_serialization::SerializeColumn(fd.GetRhs());
                    return py::make_tuple(schema_state, lhs_state, rhs_state);
                },
                // __setstate__
                [](py::tuple t) {
                    if (t.size() != 3) {
                    ...
                    auto lhs = table_serialization::DeserializeVertical(t[1].cast<py::tuple>(), schema);
                    auto rhs = table_serialization::DeserializeColumn(t[2].cast<py::tuple>(), schema);
                    return FD(lhs, rhs, schema);
                }));
