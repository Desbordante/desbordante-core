...
void BindSplit(py::module_& main_module) {
    ...
    py::class_<model::DDString>(dd_module, "DD")
            ...
            .def(py::pickle(
            // __getstate__
            [](model::DDString const &dd_str) -> py::tuple {
                std::vector<py::tuple> left_vec;
                for (auto const &c : dd_str.left) {
                    left_vec.push_back(py::make_tuple(c.column_name, c.constraint.lower_bound, c.constraint.upper_bound));
                }
                std::vector<py::tuple> right_vec;
                for (auto const &c : dd_str.right) {
                    right_vec.push_back(py::make_tuple(c.column_name, c.constraint.lower_bound, c.constraint.upper_bound));
                }
                return py::make_tuple(left_vec, right_vec);
            },
            // __setstate__
            [](py::tuple t) -> model::DDString {
                if (t.size() != 2)
                    throw std::runtime_error("Invalid state for DDString pickle!");
                std::vector<py::tuple> left_vec = t[0].cast<std::vector<py::tuple>>();
                std::vector<py::tuple> right_vec = t[1].cast<std::vector<py::tuple>>();
                ...
                for (auto const &item : left_vec) {
                    ...
                    left_list.push_back(model::DFStringConstraint(col_name, lower_bound, upper_bound));
                }
                ...
                for (auto const &item : right_vec) {
                    ...
                    double lower_bound = item[1].cast<double>();
                    double upper_bound = item[2].cast<double>();
                    right_list.push_back(model::DFStringConstraint(col_name, lower_bound, upper_bound));
                }
                model::DDString dd;
                dd.left = left_list;
                dd.right = right_list;
