...
void BindMd(py::module_& main_module) {
    ...
    py::class_<MD>(md_module, "MD")
            ...
            .def(py::pickle(
                // __getstate__
                [](MD const& md_obj) {
                    auto left_schema_state = table_serialization::SerializeRelationalSchema(md_obj.GetLeftSchema());
                    auto right_schema_state = table_serialization::SerializeRelationalSchema(md_obj.GetRightSchema());
                    py::list match_list;
                    auto matches_ptr = md_obj.GetColumnMatches();
                    if (matches_ptr) {
                        for (auto const& cm : *matches_ptr) {
                            match_list.append(py::make_tuple(cm.left_col_index, cm.right_col_index, cm.name));
                        }
                    }
                    py::list lhs_list;
                    for (auto const& csc : md_obj.GetLhs()) {
                        auto match_idx = csc.GetColumnMatchIndex();
                        auto db = csc.GetDecisionBoundary();
                        auto maybe_max = csc.GetMaxDisprovedBound();
                        bool has_max = maybe_max.has_value();
                        double max_val = has_max ? maybe_max.value() : 0.0;
                        lhs_list.append(py::make_tuple(match_idx, db, has_max, max_val));
                    }
                    auto [rhs_idx, rhs_bound] = md_obj.GetRhs();
                    auto rhs_tuple = py::make_tuple(rhs_idx, rhs_bound);
                    return py::make_tuple(left_schema_state, right_schema_state, match_list, lhs_list, rhs_tuple);
                },
                // __setstate__
                [](py::tuple st) {
                    if (st.size() != 5) {
                    ...
                    auto left_schema  = table_serialization::DeserializeRelationalSchema(st[0].cast<py::tuple>());
                    auto right_schema = table_serialization::DeserializeRelationalSchema(st[1].cast<py::tuple>());
                    ...
                    for (auto item : match_list) {
                        ...
                        matches_ptr->emplace_back(l_idx, r_idx, name);
                    }
                    ...
                    for (auto item : lhs_list) {
                        ...
                        bool has_max = tpl[2].cast<bool>();
                        double max_val = tpl[3].cast<double>();
                        std::optional<model::md::DecisionBoundary> maybe_max;
                        if (has_max) {
                            maybe_max = max_val;
                        }
                        lhs_vec.emplace_back(maybe_max, match_idx, dec_bound);
                    }
                    ...
                    model::MD md_restored(left_schema, right_schema, matches_ptr, std::move(lhs_vec), std::move(rhs_classifier));
                    return md_restored;
                }));
