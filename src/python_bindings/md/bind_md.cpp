...
void BindMd(py::module_& main_module) {
    ...
    py::class_<MD>(md_module, "MD")
            ...
            .def(py::pickle(
                    // __getstate__
                    [](MD const& md_obj) {
                        py::tuple left_schema_state = table_serialization::SerializeRelationalSchema(md_obj.GetLeftSchema().get());
                        py::tuple right_schema_state = table_serialization::SerializeRelationalSchema(md_obj.GetRightSchema().get());
                        py::list match_list;
                        std::shared_ptr<std::vector<md::ColumnMatch> const> matches_ptr = md_obj.GetColumnMatches();
                        if (matches_ptr) {
                    ...
                    [](py::tuple st) {
                        ...
                        std::shared_ptr<RelationalSchema const> left_schema  = table_serialization::DeserializeRelationalSchema(st[0].cast<py::tuple>());
                        std::shared_ptr<RelationalSchema const> right_schema = table_serialization::DeserializeRelationalSchema(st[1].cast<py::tuple>());
                        ...
                        for (auto item : lhs_list) {
                            ...
                            std::optional<model::md::DecisionBoundary> restored_maybe_max = tpl[2].cast<std::optional<model::md::DecisionBoundary>>();
