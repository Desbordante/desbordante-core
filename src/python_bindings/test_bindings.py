...
OptionContainer = namedtuple("OptionContainer", ["load_options", "execute_options"])
FailureCaseContainer = namedtuple("FailureCaseContainer", ["path", "options"])

ONLY_NULL_EQUAL_NULL_OPTION_CONTAINER = OptionContainer(
    {"table": ("WDC_satellites.csv", ",", True), "is_null_equal_null": False}, {}
)

MAXFEM_OPTION_CONTAINER = OptionContainer({"sequence": "maxfem_baseline.txt"}, {})
...
def get_apriori_load_container(load_options):
    final_load_options = load_options.copy()
    final_load_options.update({"table": ("TestWide.csv", ",", True)})
    return OptionContainer(final_load_options, {})
...
ALGO_CORRECT_OPTIONS_INFO = [
    (desb.fem.MaxFEM, [MAXFEM_OPTION_CONTAINER]),
    ...
    (
        ...
        [
            ...
            OptionContainer(
                {
                    "table": ("rules-kaggle-rows.csv", ",", True),
                    "input_format": "singular",
                    "tid_column_index": 0,
                    "item_column_index": 1,
                },
...
METRIC_VERIFIER_FAILURE_CASES = [
    ...
    FailureCaseContainer(
        "TestLong.csv", {"metric": "levenshtein", "rhs_indices": [1, 2]}
    ),
...
class TestPythonBindings(unittest.TestCase):
    def _test_correct_option_setting(self, algo, options: dict):
        testing_algo = algo()
        testing_algo.load_data(**options.load_options)
        ...
        for name, value in chain(
            options.execute_options.items(), options.load_options.items()
        ):
            if name in ["table", "sequence"]:
                continue
    ...
    def test_correct_load(self):
        for algo, option_containers in ALGO_CORRECT_OPTIONS_INFO:
            for option in option_containers:
                with self.subTest(
                    msg=f"testing setting correct options for {algo.__name__}"
                ):
                    self._test_correct_option_setting(algo, option)
