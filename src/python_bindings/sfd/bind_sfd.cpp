    ...
    py::class_<Correlation>(sfd_module, "Correlation")
            ...
            .def(py::pickle(
                    ...
                    [](py::tuple t) {
                        ...
                        std::shared_ptr<RelationalSchema> dummy_schema = std::make_shared<RelationalSchema>("__dummy__");
                        Column lhs_col = table_serialization::DeserializeColumn(t[0].cast<py::tuple>(), dummy_schema.get());
                        Column rhs_col = table_serialization::DeserializeColumn(t[1].cast<py::tuple>(), dummy_schema.get());
