...
void BindGSpan(pybind11::module_& main_module) {
    ...
    pybind11::class_<gspan::FrequentSubgraph>(gspan_module, "FrequentSubgraph")
        ...
        .def("__hash__", [](gspan::FrequentSubgraph const& self) {
            return gspan::FrequentSubgraph::Hash{}(self);
        })
        .def("__eq__", [](gspan::FrequentSubgraph const& self, gspan::FrequentSubgraph const& other) {
            return self == other;
        });

    BindPrimitiveNoBase<GSpan>(gspan_module, "GSpan").def("get_frequent_subgraphs", &GSpan::GetFrequentSubgraphs);
}
