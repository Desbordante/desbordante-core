...
int main(int argc, char const* argv[]) {
    ...
    ac_options.add_options()
        (posr::BinaryOperation, po::value<char>(&bin_operation)->default_value(bin_operation),
         "one of availible operations: /, *, +, - ")
        (posr::Fuzziness, po::value<double>(&fuzziness)->default_value(0.15),
         "fraction of exceptional records")
        (posr::FuzzinessProbability, po::value<double>(&p_fuzz)->default_value(p_fuzz),
         "probability, the fraction of exceptional records that lie outside the "
         "bump intervals is at most Fuzziness")
        (posr::Weight, po::value<double>(&weight)->default_value(weight),
         "value between 0 and 1. Closer to 0 - many short intervals. "
         "Closer to 1 - small number of long intervals")
        (posr::BumpsLimit, po::value<size_t>(&bumps_limit)->default_value(bumps_limit),
         "max considered intervals amount. Pass 0 to remove limit")
        (posr::IterationsLimit, po::value<size_t>(&iterations_limit)->default_value(iterations_limit),
         "limit for iterations of sampling")
        (posr::PairingRule, po::value<std::string>(&pairing_rule)->default_value(pairing_rule),
