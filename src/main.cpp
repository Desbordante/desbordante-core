...
int main(int argc, char const* argv[]) {
    ...
    desc.add_options()("help", "print help")("algo", po::value<std::string>(&alg),
                                             "algorithm [pyro|tane|fastfds|fdmine|dfd|fdep]")(
        ...
        "CSV header presence flag [true|false]. Default true")(
        "seed", po::value<int>(&seed), "RNG seed")("error", po::value<double>(&error),
                                                   "error for AFD algorithms. Default 0.01")(
        "maxLHS", po::value<unsigned int>(&max_lhs),
        (std::string("max considered LHS size. Default: ") + std::to_string((unsigned int)-1))
            .c_str())("threads", po::value<unsigned int>(&parallelism));
    ...
    if (alg == "pyro") {
        algorithm_instance =
            std::make_unique<Pyro>(path, separator, has_header, seed, error, max_lhs, parallelism);
    } else if (alg == "tane") {
        ...
        algorithm_instance =
            std::make_unique<FastFDs>(path, separator, has_header, max_lhs, parallelism);
