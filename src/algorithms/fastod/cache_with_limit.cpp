...
template <typename K, typename V>
bool CacheWithLimit<K, V>::Contains(const K& key) const noexcept {
    return entries_.find(key) != entries_.end();
}

template <typename K, typename V>
const V& CacheWithLimit<K, V>::Get(const K& key) const noexcept {
    return entries_[key];
}

template <typename K, typename V>
void CacheWithLimit<K, V>::Set(const K& key, const V& value) {
    if (Contains(key)) {
