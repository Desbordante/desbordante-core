...
private:
    ...
    std::unique_ptr<structures::VerticalMap<VerticalInfo>> local_visitees_ = nullptr;
    std::unique_ptr<structures::VerticalMap<VerticalInfo>> global_visitees_;
    std::set<DependencyCandidate, DependencyCandidateComp> launch_pads_;
    std::unique_ptr<structures::VerticalMap<DependencyCandidate>> launch_pad_index_;
    std::list<DependencyCandidate> deferred_launch_pads_;
    std::unique_ptr<structures::VerticalMap<Vertical>> scope_;
    ...
    std::optional<Vertical> TrickleDownFrom(DependencyCandidate min_dep_candidate,
                                            ...
                                            structures::VerticalMap<VerticalInfo>* global_visitees,
                                            double boost_factor);

    // CAREFUL: resets globalVisitees_, therefore SearchSpace could become invalidated
    std::unique_ptr<structures::VerticalMap<VerticalInfo>> MoveOutGlobalVisitees() {
        return std::move(global_visitees_);
    }
    // CAREFUL: resets localVisitees_, therefore SearchSpace could become invalidated
    std::unique_ptr<structures::VerticalMap<VerticalInfo>> MoveOutLocalVisitees() {
        return std::move(local_visitees_);
    }
    void MoveInLocalVisitees(
            std::unique_ptr<structures::VerticalMap<VerticalInfo>> local_visitees) {
    ...
    static std::vector<Vertical> GetSubsetDeps(
    ...
    static std::string FormatArityHistogram(structures::VerticalMap<int*>) = delete;
...
public:
    ...
    SearchSpace(int id, std::unique_ptr<DependencyStrategy> strategy,
                std::unique_ptr<structures::VerticalMap<Vertical>> scope,
                std::unique_ptr<structures::VerticalMap<VerticalInfo>> global_visitees,
        ...
        : strategy_(std::move(strategy)),
          ...
          launch_pad_index_(std::make_unique<structures::VerticalMap<DependencyCandidate>>(schema)),
    ...
    SearchSpace(int id, std::unique_ptr<DependencyStrategy> strategy,
        ...
        : SearchSpace(id, std::move(strategy), nullptr,
                      std::make_unique<structures::VerticalMap<VerticalInfo>>(schema), schema,
