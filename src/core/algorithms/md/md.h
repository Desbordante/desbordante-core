...
public:
    ...
    [[nodiscard]] std::shared_ptr<RelationalSchema const> GetLeftSchema() const {
        return left_schema_;
    }

    [[nodiscard]] std::shared_ptr<RelationalSchema const> GetRightSchema() const {
        return right_schema_;
    }

    [[nodiscard]] std::shared_ptr<std::vector<md::ColumnMatch> const> GetColumnMatches() const {
