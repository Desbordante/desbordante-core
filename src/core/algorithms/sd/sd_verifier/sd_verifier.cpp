...
void SDVerifier::InitExactGap(size_t n, std::vector<double> const& values,
    ...
    for (size_t i = 1; i < n; ++i) {
        if (!AlmostEqual(remainders[i].first, remainders[i - 1].first)) {
            cid++;
        }
    ...
    if (cid > 0 && AlmostEqual(g1_ + remainders[0].first, remainders.back().first)) {
    ...
    while (current_idx >= 0) {
        ...
        if (previous_idx == -1) {
            ...
            if (dcost > 1) {
                ...
                if (!is_exact_gap && !AlmostEqual(g1_, 0.0)) {
...
long SDVerifier::CalculateOps(std::vector<double> const& values,
    ...
    bool is_exact_gap = (g2_ > 0 && AlmostEqual(g2_, g1_));
    ...
    for (size_t i = 1; i < n; ++i) {
        ...
        if (is_exact_gap) {
            ...
            if (rank_i > 0) {
                ...
                if (min_res.has_value()) {
                    ...
                    long cost = min_res->t_val + static_cast<long>(i) - 1 - min_res->j_val +
                                (dcost - 1);
                    min2_cost = cost;
        ...
        if (AlmostEqual(g1_, 0.0) && min3_cost != -1 && min3_cost <= t[i]) {
            t[i] = min3_cost;
            t_prev[i] = min3_j;
        }
