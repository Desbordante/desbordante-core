...
IdList::IdList(std::shared_ptr<std::vector<ItemsetId> const> itemset_counts)
...
boost::dynamic_bitset<> IdList::EqualOperation(boost::dynamic_bitset<> const& bitset1,
                                               boost::dynamic_bitset<> const& bitset2) {
    return bitset1 & bitset2;
}

boost::dynamic_bitset<> IdList::AfterOperation(boost::dynamic_bitset<> const& bitset1,
                                               boost::dynamic_bitset<> const& bitset2) {
...
std::optional<IdList> IdList::Join(IdList const& other, bool is_equal, MinSupport min_sup) const {
    ...
    for (std::size_t sid = common_sequences.find_first(); sid != boost::dynamic_bitset<>::npos;
         sid = common_sequences.find_next(sid)) {
        auto const& this_bitset = sequence_itemset_entries_[sid];
        auto const& other_bitset = other.sequence_itemset_entries_[sid];
