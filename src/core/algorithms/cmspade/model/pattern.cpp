...
ItemAbstractionPair const& Pattern::GetLastElement() const {
    return elements_.back();
}

ItemAbstractionPair const& Pattern::GetPenultimateElement() const {
...
int Pattern::CompareTo(Pattern const& other) const {
...
bool Pattern::IsPrefix(Pattern const& other) const {
    if (other.Size() <= Size()) {
        return false;
    }
    for (std::size_t i = 0; i < elements_.size(); i++) {
