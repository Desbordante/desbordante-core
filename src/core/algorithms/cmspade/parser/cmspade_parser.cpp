...
bool CMSpadeParser::ParseNextSequence(SequenceId sequence_id, std::unique_ptr<Sequence>& sequence) {
    std::string line;
...
void CMSpadeParser::ParseSequenceLine(std::string const& line, SequenceId sequence_id,
                                      std::unique_ptr<Sequence>& sequence) {
    ...
    while (iss >> token) {
        ...
        if (token == "-1") {
            if (current_itemset && !current_itemset->Empty()) {
