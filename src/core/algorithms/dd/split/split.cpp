...
unsigned long long Split::ExecuteInternal() {
    ...
    spdlog::info("Calculated distances");
    ...
    spdlog::info("Calculated tuple pairs");
    elapsed_milliseconds = std::chrono::duration_cast<std::chrono::milliseconds>(
            std::chrono::system_clock::now() - start_time);
    spdlog::debug("Current time: {}", elapsed_milliseconds.count());
    spdlog::info("Minimum and maximum distances for each column with non-empty search space:");

    for (model::ColumnIndex index = 0; index < num_columns_; index++)
        spdlog::info("{}: {}, {}", input_table_->GetColumnName(non_empty_cols_[index]),
                     min_max_dif_[index].lower_bound, min_max_dif_[index].upper_bound);

    unsigned const search_size = ReduceDDs(start_time);

    spdlog::debug("Reduced dependencies");

    unsigned num_cycles = RemoveRedundantDDs();

    spdlog::info("Removed redundant dependencies");
    spdlog::debug("Cycles: {}", num_cycles);

    num_cycles = RemoveTransitiveDDs();

    spdlog::info("Removed transitive dependencies");
    spdlog::debug("Cycles: {}", num_cycles);
    spdlog::info("Search space size: {}", search_size);

    PrintResults();

    elapsed_milliseconds = std::chrono::duration_cast<std::chrono::milliseconds>(
            std::chrono::system_clock::now() - start_time);
    spdlog::info("Algorithm time: {}", elapsed_milliseconds.count());
...
unsigned Split::ReduceDDs(auto const& start_time) {
    ...
    for (model::ColumnIndex index = 0; index < num_columns_; index++) {
        ...
        spdlog::debug("Calculated search spaces for column {}",
                      input_table_->GetColumnName(non_empty_cols_[index]));
        auto elapsed_milliseconds = std::chrono::duration_cast<std::chrono::milliseconds>(
                std::chrono::system_clock::now() - start_time);
        spdlog::debug("Current time: {}", elapsed_milliseconds.count());
        spdlog::debug("Lhs and rhs search space sizes: {} {}", search.size(), dfs_y.size());
        spdlog::debug("Number of verifications for each df in rhs:");
        ...
        elapsed_milliseconds = std::chrono::duration_cast<std::chrono::milliseconds>(
                std::chrono::system_clock::now() - start_time);
        spdlog::info("Reduced dependencies with their rhs on column {}",
                     input_table_->GetColumnName(non_empty_cols_[index]));
        spdlog::debug("Current time: {}", elapsed_milliseconds.count());
    }
    return search_size;
...
void Split::PrintResults() {
    ...
    spdlog::info("Minimal cover size: {}", result_strings.size());
    for (auto const& result_str : result_strings) {
        spdlog::debug("{}", result_str.ToString());
    }
}
