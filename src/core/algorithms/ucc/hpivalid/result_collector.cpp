...
ResultCollector::ResultCollector(double timeout)
    : timeout_(timeout),
      ucc_count_(0),
      diff_sets_final_(0),
      timer_beg_(timer::TimerName::num_of_timers),
      timer_end_(timer::TimerName::num_of_timers),
      timer_elapsed_(timer::TimerName::num_of_timers, 0),
      diff_sets_(0),
...
bool ResultCollector::UCCFound(Edge const& ucc) {
    ...
    return std::chrono::duration_cast<std::chrono::duration<double>>(
                   clock::now() - timer_beg_[timer::TimerName::total])
                   .count() <= timeout_;
}
...
void ResultCollector::StartTimer(timer::TimerName timer) {
    timer_beg_[timer] = clock::now();
}

void ResultCollector::StopTimer(timer::TimerName timer) {
    timer_end_[timer] = clock::now();
    timer_elapsed_[timer] += std::chrono::duration_cast<std::chrono::milliseconds>(
                                     timer_end_[timer] - timer_beg_[timer])
                                     .count();
}

unsigned long long ResultCollector::Time(timer::TimerName timer) const {
    return timer_elapsed_[timer];
}
