...
private:
    ...
    [[nodiscard]] UCCTreeVertex* GetChild(std::size_t pos) const noexcept {
    ...
    void RemoveRecursive(boost::dynamic_bitset<> const& ucc, std::size_t cur_bit);
    [[nodiscard]] bool FindUCCOrGeneralizationRecursive(boost::dynamic_bitset<> const& ucc,
                                                        std::size_t cur_bit) const;
    ...
    explicit UCCTreeVertex(std::size_t num_attributes, bool is_ucc)
        : num_attributes_(num_attributes), is_ucc_(is_ucc) {}

    static std::unique_ptr<UCCTreeVertex> Create(std::size_t num_attributes, bool is_ucc) {
...
public:
    ...
    [[nodiscard]] bool ContainsChildAt(std::size_t pos) const noexcept {
        assert(pos < children_.size());
        return children_[pos] != nullptr;
    }

    [[nodiscard]] UCCTreeVertex* GetChildIfExists(std::size_t pos) const {
        if (children_.empty()) {
            return nullptr;
        }
