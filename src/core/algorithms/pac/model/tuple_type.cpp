...
namespace pac::model {
::model::CompareResult TupleType::CompareComponents(std::size_t const component_idx,
                                                    std::byte const* x, std::byte const* y) {
    assert(component_idx < types_.size());
    ...
    return types_[component_idx]->Compare(x, y);
}
