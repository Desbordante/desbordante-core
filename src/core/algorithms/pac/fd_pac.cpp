...
std::string FDPAC::ToShortString() const {
    ...
    oss << ColumnNamesToString(lhs_column_names_) << " -(" << DoublesToString(epsilons_) << ", "
        << delta_ << ")-> " << ColumnNamesToString(rhs_column_names_);
    return oss.str();
}
...
std::string FDPAC::ToLongString() const {
    ...
    std::ostringstream oss;
    oss << "FD PAC d(" << ColumnNamesToString(lhs_column_names_) << ") ≤ "
        << DoublesToString(lhs_Deltas_) << "}) => Pr(d(" << ColumnNamesToString(rhs_column_names_)
        << ") ≤ " << DoublesToString(epsilons_) << ") ≥ " << delta_;
    return oss.str();
}
...
bool FDPAC::operator==(FDPAC const& other) const {
    ...
    return epsilons_equal && approx_equal(delta_, other.delta_) && lhs_deltas_equal &&
           lhs_indices_ == other.lhs_indices_ && rhs_indices_ == other.rhs_indices_;
}
}  // namespace model
