...
void Faida::LoadINDAlgorithmDataInternal() {
    ...
    spdlog::debug("Preprocessing time: {}", prepr_time_);
}
    ...
    for (TableIndex table_idx = 0; table_idx < data.GetStores().size(); table_idx++) {
        ...
        for (ColumnIndex col_idx = 0; col_idx < num_columns; col_idx++) {
            if (ignore_const_cols_ && store.IsConstantCol(col_idx)) {
                spdlog::debug("Ignoring constant column with idx {}", col_idx);
                continue;
            }
            if (ignore_null_cols_ && store.IsNullCol(col_idx)) {
                spdlog::debug("Ignoring null column with idx {}", col_idx);
                continue;
            }
...
unsigned long long Faida::ExecuteInternal() {
    ...
    spdlog::debug("Found {} INDs on level {}", last_result.size(), level_num);
    ...
    while (!last_result.empty() && ++level_num != max_arity_) {
        ...
        spdlog::debug("Extracted {} CCs", combinations.size());
    ...
    spdlog::debug("Uncertain checks:\t{}", inclusion_tester_->GetNumUncertainChecks());
    spdlog::debug("\nOverall time\t{}", millis + prepr_time_);
    spdlog::debug("Time (without preprocessing):\t{}", millis);
    spdlog::debug("\tInserting:\t{}", insert_time_);
    spdlog::debug("\tChecking:\t{}", check_time_);
...
void Faida::InsertRows(faida::IInclusionTester::ActiveColumns const& active_columns,
    ...
    for (auto const& [curr_table, columns] : active_columns) {
        ...
        spdlog::debug("Inserted {} rows from table {}", row_idx, curr_table);
    }
    ...
    insert_time_ += millis;
    spdlog::debug("Insert rows time:\t{}", millis);
}
...
std::vector<faida::SimpleIND> Faida::TestCandidates(std::vector<SimpleIND> const& candidates) {
    ...
    size_t const millis = elapsed_milliseconds.count();
    check_time_ += millis;
    spdlog::debug("Candidates check time:\t{}", millis);
    return result;
}
