...
void CFUN::ExecuteInternal() {
    LOG_INFO("Starting CFUN with min_support = {}", min_support_);
    ...
    LOG_INFO("CFUN completed with {} CFDs found in {} ms", cfd_list_.size(),
...
std::vector<Quadruple::Partition> CFUN::CalculateMonoPartitions() const {
    ...
    for (size_t row = 0; row < cfd_relation_->GetNumRows(); ++row) {
