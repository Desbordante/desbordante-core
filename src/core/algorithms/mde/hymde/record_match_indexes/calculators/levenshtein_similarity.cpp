...
ComparisonResult LevenshteinComparerCreator::Comparer::operator()(PartitioningValue const& l,
    ...
    if (max_dist != 0) {
        std::size_t lim = max_dist * (1 - sim_cutoff_);
        std::size_t dist = LevenshteinDistance(&l, &r, buf.get(), r_buf, lim, max_dist);
        similarity = (max_dist - dist) / static_cast<ComparisonResult>(max_dist);
        if (similarity < sim_cutoff_) similarity = orders::SimilarityLe{}.LeastElement();
    }
    return similarity;
}
}  // namespace algos::hymde::record_match_indexes::calculators::levenshtein_similarity
