...
std::vector<std::unordered_map<int, vertex_t>> SubgraphIsomorphisms(DFSCode& code,
                                                                    graph_t const& graph) {
    ...
    for (ExtendedEdge const& ee : code.GetExtendedEdges()) {
        ...
        for (auto& iso : isoms) {
            ...
            if (v1 < v2) {
                std::vector<vertex_t> mapped_vertices;
                mapped_vertices.reserve(iso.size());
                for (auto& entry : iso) {
                    mapped_vertices.push_back(entry.second);
                }
                ...
                for (auto edge : boost::make_iterator_range(boost::out_edges(mapped_v1, graph))) {
                    ...
                    if (v2_label == graph[mapped_v2].label &&
                        (std::find(mapped_vertices.begin(), mapped_vertices.end(), mapped_v2) ==
                         mapped_vertices.end()) &&
                ...
                bool is_neighbors = boost::edge(mapped_v1, mapped_v2, graph).second;
                auto& mapped_edge = graph[boost::edge(mapped_v1, mapped_v2, graph).first];
                if (is_neighbors && edge_label == mapped_edge.label) {
...
graph_t CreateGraphFromDFSCode(DFSCode code) {
    graph_t result;
...
void GSpan::GSpanDFS(gspan::DFSCode code, std::unordered_set<int> graph_ids) {
    ...
    if (code.Size() == static_cast<size_t>(max_number_of_edges_ - 1)) {
...
GSpan::RightMostPathExtensions(DFSCode code, std::unordered_set<int> graph_ids) {
    LOG_TRACE("Computing extensions: pattern size={}, graphs={}", code.Size(), graph_ids.size());
    std::unordered_map<ExtendedEdge, std::unordered_set<int>, ExtendedEdge::Hash> extensions;
    if (code.Empty()) {
        LOG_TRACE("Empty pattern, collecting initial edges");
        for (auto graph_id : graph_ids) {
            auto& graph = graph_database_[graph_id];

            if (code.Size() >= boost::num_edges(graph)) {
                continue;
            }
            ...
            for (auto vertex : boost::make_iterator_range(boost::vertices(graph))) {
                for (auto edge : boost::make_iterator_range(boost::out_edges(vertex, graph))) {
                    ...
                    auto neighbor = boost::target(edge, graph) == vertex
                                            ? boost::source(edge, graph)
                                            : boost::target(edge, graph);
                    ...
                    extensions[ee].insert(graph_id);
                }
            }
        }
    } else {
        LOG_TRACE("Extending pattern along rightmost path");
        int rightmost = code.GetRightMost();
        for (int graph_id : graph_ids) {
            ...
            std::vector<std::unordered_map<int, vertex_t>> isoms =
                    SubgraphIsomorphisms(code, graph);
            LOG_TRACE("Graph {}: {} embeddings found", graph_id, isoms.size());
            for (auto& isom : isoms) {
                // Backward extensions from rightmost child
                std::unordered_map<vertex_t, int> inverted_isom;
                for (auto& entry : isom) {
                    inverted_isom[entry.second] = entry.first;
                }

                vertex_t mapped_rightmost = isom[rightmost];
                ...
                for (auto edge :
                    ...
                    vertex_t neighbor = (boost::source(edge, graph) == mapped_rightmost)
                                                ? boost::target(edge, graph)
                                                : boost::source(edge, graph);
                    auto inv_it = inverted_isom.find(neighbor);
                    if (inv_it == inverted_isom.end()) {
                        continue;
                    }
                    auto inverted = inv_it->second;
                    if (code.OnRightMostPath(inverted) && code.NotPreOfRM(inverted) &&
                        ...
                        extensions[ee].insert(graph[boost::graph_bundle].id);
                    }
                }
                // Forward extensions from nodes on rightmost path
                std::vector<vertex_t> mapped_vertices;
                mapped_vertices.reserve(isom.size());
                for (auto& entry : isom) {
                    mapped_vertices.push_back(entry.second);
                }
                ...
                for (auto vertex : code.GetRightMostPath()) {
                    ...
                    for (auto edge :
                        ...
                        vertex_t neighbor = (boost::source(edge, graph) == mapped_vertex)
                                                    ? boost::target(edge, graph)
                                                    : boost::source(edge, graph);
                        if (std::find(mapped_vertices.begin(), mapped_vertices.end(), neighbor) ==
                            mapped_vertices.end()) {
                            int edge_label = graph[edge].label;

                            ExtendedEdge ee(Vertex(vertex, mapped_vertex_label),
                                            Vertex(rightmost + 1, graph[neighbor].label),
                                            edge_label);
                            extensions[ee].insert(graph[boost::graph_bundle].id);
                        }
                    }
    ...
    LOG_TRACE("Computed {} distinct extensions", extensions.size());
    return extensions;
}

std::unordered_set<ExtendedEdge, ExtendedEdge::Hash> GSpan::RightMostPathExtensionsFromSingle(
        DFSCode code, graph_t const& graph) {
    LOG_TRACE("Computing single-graph extensions: pattern size={}, vertices={}", code.Size(),
            ...
            for (auto vertex : code.GetRightMostPath()) {
                ...
                for (auto edge :
                    ...
                    vertex_t neighbor = (boost::source(edge, graph) == mapped_vertex)
                                                ? boost::target(edge, graph)
                                                : boost::source(edge, graph);
                    if (!inverted_isom.contains(neighbor)) {
                        int edge_label = graph[edge].label;
                        ExtendedEdge ee(Vertex(vertex, mapped_vertex_label),
...
bool GSpan::IsCanonical(DFSCode code) {
    LOG_TRACE("Checking canonicity: pattern size={}", code.Size());
    DFSCode canon;
    for (size_t i = 0; i < code.Size(); i++) {
        std::unordered_set<ExtendedEdge, ExtendedEdge::Hash> extensions =
                RightMostPathExtensionsFromSingle(canon, CreateGraphFromDFSCode(code));
