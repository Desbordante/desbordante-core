...
std::vector<gspan::graph_t> ReadGraphs(std::istream& stream) {
    ...
    bool has_next_graph = std::getline(stream, line) && line.starts_with("t");
    while (has_next_graph) {
        gspan::graph_t graph;
        ...
        std::map<int, vertex_t> id_to_desc;
        ...
        has_next_graph = (stream.eof()) ? false : true;
    }
...
std::vector<gspan::graph_t> ReadGraphs(std::filesystem::path const& path) {
    ...
    std::vector<gspan::graph_t> result = ReadGraphs(file);
    file.close();
