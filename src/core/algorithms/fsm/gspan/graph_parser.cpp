...
std::vector<gspan::graph_t> ReadGraphs(std::istream& stream) {
    ...
    try {
        while (stream) {
            ...
            while (std::getline(stream, line)) {
                ...
                if (line.starts_with("v")) {
                    ...
                    auto edge1 = boost::add_edge(vertex1, vertex2, graph);
                    graph[edge1.first].label = edge_label;
                    graph[edge1.first].id = edge_id;
        ...
        throw std::runtime_error("Failed to parse graph at line " + std::to_string(line_number) +
                                 ": " + e.what());
    }
