...
#include "algorithms/association_rules/ar_verifier/model/rule_coverage.h"
...
private:
    ...
    std::unordered_map<size_t, util::RuleCoverage> rule_coverage_coefficients_;
    std::unordered_map<std::string, model::PLI::Cluster> clusters_violating_ar_;

    static ClusterPriority CalculateClusterPriority(util::RuleCoverage const& coverage);
