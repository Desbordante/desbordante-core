...
#include "config/ar_minimum_conf/option.h"
#include "config/ar_minimum_support/option.h"
#include "config/names_and_descriptions.h"
#include "config/option_using.h"
#include "config/tabular_data/input_table/option.h"
#include "config/transactional_data/option.h"
...
void ARVerifier::RegisterOptions() {
    ...
    auto sing_eq = [](::model::InputFormatType input_format) {
        return input_format == +::model::InputFormatType::singular;
    };
    auto tab_eq = [](::model::InputFormatType input_format) {
        return input_format == +::model::InputFormatType::tabular;
    };
    ...
    RegisterOption(config::kInputFormatOpt(&transactional_data_params_.input_format_type)
                           .SetConditionalOpts({{sing_eq, {kTIdColumnIndex, kItemColumnIndex}},
                                                {tab_eq, {kFirstColumnTId}}}));
...
unsigned long long ARVerifier::ExecuteInternal() {
    ...
    LOG(DEBUG) << "\tInput format: " << transactional_data_params_.input_format_type;
