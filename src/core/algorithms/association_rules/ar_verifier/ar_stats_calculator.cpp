...
ClusterPriority ARStatsCalculator::CalculateClusterPriority(util::RuleCoverage const& coverage) {
...
void ARStatsCalculator::CalculateRuleCoverageCoefficients() {
    for (auto const& [id, itemset] : data_->GetTransactions()) {
        if (auto const coverage =
                    util::RuleCoverage(itemset.GetItemsIDs(), rule_.left, rule_.right);
