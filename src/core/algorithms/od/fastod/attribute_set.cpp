...
AttributeSet::AttributeSet(std::size_t attribute) noexcept : set_({attribute}), value_(1 << attribute) {}

AttributeSet::AttributeSet(const std::vector<std::size_t>& attributes) noexcept :
    set_(attributes.cbegin(), attributes.cend()),
    value_(std::accumulate(attributes.cbegin(), attributes.cend(), 0, [](const unsigned long long& acc, const std::size_t& curr){ return acc + (1 << curr); })) {}

AttributeSet::AttributeSet(const std::set<std::size_t>& set) noexcept :
    set_(set),
    value_(std::accumulate(set.cbegin(), set.cend(), 0, [](const unsigned long long& acc, const std::size_t& curr){ return acc + (1 << curr); })) {}

bool AttributeSet::ContainsAttribute(std::size_t attribute) const noexcept {
    return set_.find(attribute) != set_.end();
}

AttributeSet AttributeSet::AddAttribute(std::size_t attribute) const noexcept {
    if (ContainsAttribute(attribute)) {
        return *this;
    }

    std::set<std::size_t> new_set = set_;
    new_set.insert(attribute);

    return AttributeSet(std::move(new_set));
}

AttributeSet AttributeSet::DeleteAttribute(std::size_t attribute) const noexcept {
    if (!ContainsAttribute(attribute)) {
        return *this;
    }
...
std::set<std::size_t>::iterator AttributeSet::begin() const noexcept {
    return set_.begin();
}

std::set<std::size_t>::iterator AttributeSet::end() const noexcept {
    return set_.end();
}
