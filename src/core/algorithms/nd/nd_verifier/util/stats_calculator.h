...
public:
    ...
    [[nodiscard]] auto GetLhsFrequencies() const {
        assert(lhs_values_->size() == lhs_frequencies_->size());

        std::unordered_map<std::string, size_t> result;
        for (size_t i{0}; i < lhs_values_->size(); ++i) {
            result.emplace(lhs_values_->at(i), lhs_frequencies_->at(i));
        }
    ...
    [[nodiscard]] auto GetRhsFrequencies() const {
        assert(rhs_values_->size() == rhs_frequencies_->size());

        std::unordered_map<std::string, size_t> result;
        for (size_t i{0}; i < rhs_values_->size(); ++i) {
            result.emplace(rhs_values_->at(i), rhs_frequencies_->at(i));
        }
