...
private:
    ...
    // Cached data:
    std::unique_ptr<std::vector<size_t>> most_frequent_rhs_value_codes_{nullptr};
    ...
    std::vector<size_t> const& GetMostFrequentRhsValueCodes();
...
public:
    ...
    [[nodiscard]] size_t GetOccurencesNumber() const;

    /// @brief Get Lhs value
    [[nodiscard]] std::string GetLhsValue() const {
        return lhs_values_->at(highlight_lhs_);
    }
