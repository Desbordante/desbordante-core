...
private:
    ...
    std::unique_ptr<std::vector<std::size_t>> occurences_indices_{nullptr};
    ...
    [[nodiscard]] std::vector<std::size_t> CalculateOccurencesIndices() const;
...
public:
    ...
    /// operation)
    std::vector<std::size_t> const& GetOccurencesIndices();

    /// @brief Get number of occurences of this highlight
    [[nodiscard]] std::size_t GetOccurencesNumber();
