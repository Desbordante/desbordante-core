...
protected:
     ...
     */
    bool CheckToken(Token const& token, std::string& tok, graph_t const& pattern,
                    std::map<vertex_t, vertex_t> const& embedding);

    /* Checks whether a list of literals is satisfied for a given pattern
     * on a given embedding
     */
    bool Satisfied(std::vector<Literal> const& literals, graph_t const& pattern,
                   std::map<vertex_t, vertex_t> const& embedding);
     ...
     */
    void AddCompacted(
            std::vector<std::pair<std::vector<Literal>, Literal>>& rules, graph_t& pattern,
            std::vector<std::pair<std::vector<Literal>, std::vector<Literal>>>& forbidden_rules);

    /* Checks whether the given dependency is satisfied on the graph
     * and whether it is frequent.
     */
    bool Validate(Gfd const& gfd, std::vector<std::map<vertex_t, vertex_t>> const& embeddings);

    /* Generates all the rules of the satisfied dependencies for a given pattern
     * and for a given literal on the right-hand side.
     */
    void HGenerate(std::vector<Literal>& notsatisfied, Literal const& l,
                   std::vector<std::pair<std::vector<Literal>, Literal>>& rules, graph_t& pattern,
                   std::vector<std::map<vertex_t, vertex_t>> const& embeddings,
                   std::vector<std::pair<std::vector<Literal>, std::vector<Literal>>> const&
                           forbidden_rules);

    /* Generates all the rules of the satisfied dependencies for a given pattern
     */
    std::vector<std::pair<std::vector<Literal>, Literal>> GenerateRules(
            std::vector<Literal> const& literals, graph_t& pattern,
            std::vector<std::map<vertex_t, vertex_t>> const& embeddings,
            std::vector<std::pair<std::vector<Literal>, std::vector<Literal>>> const&
                    forbidden_rules);
...
public:
    ...
    std::vector<Gfd> const& GfdList() const noexcept {
        return gfds_;
    }
};
