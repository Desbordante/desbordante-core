    ...
    for (auto const& index_pair : pairs) {
        ...
        for (auto const& fst_name : attrs_info.at(fst_label)) {
            ...
            for (auto const& snd_name : attrs_info.at(snd_label)) {
                ...
                result.push_back({fst_token, std::move(snd_token)});
            }
        }
    }
...
void Initialize(graph_t const& graph, std::set<std::string>& vertex_labels,
    ...
    BGL_FORALL_VERTICES_T(v, graph, graph_t) {
        ...
        for (std::pair<std::string const, std::string> const& attr : graph[v].attributes) {
            if (attr.first == "label") {
                continue;
            }
            ...
            if (!label_emplaced) {
                std::set<std::string> attrs = {attr.second};
                auto [it, emplaced] = label_info->second.try_emplace(attr.first, std::move(attrs));
                if (!emplaced) {
...
void AddCompacted(std::vector<SingleRule>& rules, std::vector<Gfd>& result, graph_t& pattern,
    ...
    if (!rules.empty()) {
        ...
        Gfd gfd = {pattern, prev, conclusion};
        result.push_back(std::move(gfd));
        forbidden_rules->push_back({std::move(prev), std::move(conclusion)});
    }
}
...
void AddEdge(std::set<std::string> const& edge_labels, graph_t const& graph, graph_t const& pattern,
    ...
    std::size_t vertices = boost::num_vertices(pattern);
    std::size_t max_edges = vertices * (vertices + 1) / 2;
    if (boost::num_edges(pattern) != max_edges) {
        std::vector<std::size_t> indices(vertices);
        std::iota(std::begin(indices), std::end(indices), 0);
        ...
        for (auto& label : edge_labels) {
            ...
            for (auto& embedding : embeddings) {
                if (boost::edge(embedding.at(origin), embedding.at(finish), graph).second) {
                    new_embeddings.push_back(std::move(embedding));
                }
            }
...
void AddVertex(std::set<std::string> const& vertex_labels, std::set<std::string> const& edge_labels,
    ...
    for (auto const& edge_label : edge_labels) {
        for (auto const& vertex_label : vertex_labels) {
            std::size_t pat_vertices = boost::num_vertices(pattern);
            for (std::size_t j = 0; j < pat_vertices; ++j) {
                ...
                vertex_t new_vertex = boost::add_vertex(
                        Vertex{static_cast<int>(pat_vertices), attributes}, new_pattern);
                ...
                for (auto& embedding : embeddings) {
                    ...
                    for (; adjacency_it != adjacency_end; ++adjacency_it) {
                        ...
                        Embedding new_embedding(std::move(embedding));
...
unsigned long long GfdMiner::ExecuteInternal() {
    std::size_t elapsed_time = util::TimedInvoke(&GfdMiner::MineGfds, this, graph_, k_, sigma_);
    LOG(DEBUG) << "Mined GFDs: " << gfds_.size();
    return elapsed_time;
}

void GfdMiner::MineGfds(graph_t const& graph, std::size_t const k, std::size_t const sigma) {
    ...
    while (!patterns.empty()) {
        ...
        for (std::size_t i = 0; i < patterns.size(); ++i) {
            ...
            std::size_t pat_vertices = boost::num_vertices(pattern);
            if (pat_vertices >= k || pat_vertices >= boost::num_vertices(graph)) {
                continue;
            }
