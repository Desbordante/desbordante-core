...
std::size_t GetMinSize(VertexImage const& vertex_map, std::size_t fallback_size) {
    ...
    return vertex_map.empty()
                   ? fallback_size
                   : std::ranges::min(vertex_map | std::views::transform(get_image_size));
}
...
void UpdateRules(std::size_t index, graph_t const& existed, Embedding const& iso,
                 graph_t const& new_pattern, std::vector<Rules>& new_forbidden_rules_set,
        ...
        for (auto const& vertex_label : vertex_labels) {
            for (std::size_t j = 0; j < num_pat_vertices; ++j) {
                ...
                auto modify_pattern = [&u, &num_pat_vertices, &vertex_label, &edge_label,
                    ...
                    Vertex new_v = Vertex{static_cast<int>(num_pat_vertices), attributes};
...
bool GfdMiner::CheckToken(Token const& token, std::string& tok, graph_t const& pattern,
...
bool GfdMiner::Satisfied(std::vector<Literal> const& literals, graph_t const& pattern,
                         Embedding const& embedding) {
...
void GfdMiner::AddCompacted(std::vector<SingleRule>& rules, graph_t& pattern,
...
bool GfdMiner::Validate(Gfd const& gfd, Embeddings const& embeddings) {
    bool result = false;
    VertexImage vertex_map;
...
void GfdMiner::HGenerate(std::vector<Literal>& notsatisfied, Literal const& l,
                         std::vector<SingleRule>& rules, graph_t& pattern,
                         Embeddings const& embeddings, Rules const& forbidden_rules) {
    ...
    for (std::size_t k = 1; k <= 3; ++k) {
        ...
        for (auto& subset : subsets) {
            ...
            if (Validate(gfd, embeddings)) {
                ...
                auto it = std::ranges::remove_if(notsatisfied, cond).begin();
                notsatisfied.erase(it, notsatisfied.end());
            }
        }
    }
}

std::vector<SingleRule> GfdMiner::GenerateRules(std::vector<Literal> const& literals,
    ...
    for (std::size_t j = 0; j < literals.size(); ++j) {
        ...
        Gfd gfd = {pattern, std::vector<Literal>{}, std::vector<Literal>{l}};
        if (Validate(gfd, embeddings)) {
            rules.emplace_back(std::vector<Literal>{}, l);
            continue;
        }
