...
std::vector<size_t> Measure::GetFrequencies() const {
    auto freqs = verifier_->GetFrequencies();
    std::vector<size_t> res = util::GetPreallocatedVector<size_t>(freqs.size());
    auto get_count = [](auto const& item) { return item.second; };
...
Measure::Measure(std::shared_ptr<algos::DCVerifier> verifier)
    : rel_size_(verifier->relation_->GetNumRows()),
      verifier_(verifier),
      type_to_method_map_{{MeasureType::kG1, &Measure::G1},
                          {MeasureType::kG1Norm, &Measure::G1Norm},
                          {MeasureType::kG2, &Measure::G2}} {}
