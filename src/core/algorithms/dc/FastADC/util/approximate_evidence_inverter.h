...
public:
    ApproxEvidenceInverter(PredicateBuilder& pbuilder, double threshold, EvidenceSet&& evidence_set,
        ...
        : n_predicates_(pbuilder.PredicateCount()),
          ...
          schema_(schema) {
        spdlog::debug(" [AEI] Violate at most {} tuple pairs", evi_count_ - target_);
    ...
    DenialConstraintSet BuildDenialConstraints() {
        ...
        spdlog::debug("  [AEI] Min cover size: {}", raw_dcs.size());
        ...
        spdlog::debug("  [AEI] Total DC size: {}", constraints.TotalDCSize());

        constraints.Minimize();

        spdlog::debug("  [AEI] Min DC size : {}", constraints.MinDCSize());
...
private:
    ...
    void InverseEvidenceSet() {
        spdlog::debug("  [AEI] Inverting evidences...");
