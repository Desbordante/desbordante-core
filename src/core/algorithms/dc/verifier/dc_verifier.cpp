...
unsigned long long int DCVerifier::ExecuteInternal() {
    // Is it supposed to be used like that
    // if we run the algorithms several times?
    ResetState();
    ...
    auto comp = [](std::pair<size_t, size_t> const& lhs, std::pair<size_t, size_t> const& rhs) {
        return (lhs.first != rhs.first) ? (lhs.first < rhs.first) : (lhs.second < rhs.second);
    };

    std::sort(violations_.begin(), violations_.end(), comp);
...
bool DCVerifier::VerifyMixed(dc::DC const& dc) {
    ...
    for (size_t i = 0; i < data_.front().GetNumRows(); ++i) {
        ...
        boost::optional<Point> t_point = process(s_tree, t_predicates);
        boost::optional<Point> s_point = process(t_tree, s_predicates);
        ...
        if (t_point.has_value()) t_tree.Insert(t_point.get());
        if (s_point.has_value()) s_tree.Insert(s_point.get());
    }

    return violations_.empty();
}

boost::optional<Point> DCVerifier::ProcessMixed(
    ...
    boost::optional<Point> insert_point;
    ...
    for (size_t i = 0; i < data_.front().GetNumRows(); ++i) {
        ...
        if (res_tuples.find(point) != res_tuples.end()) {
            ...
            for (auto ind : viol_indexes) {
                violations_.push_back({cur_ind, ind});
            }
