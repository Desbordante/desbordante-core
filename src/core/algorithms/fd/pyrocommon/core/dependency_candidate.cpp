...
// TODO: perhaps in Java code they meant to implement error -> arity -> lexicographical? Check
// usages
bool DependencyCandidate::FullArityErrorComparator(DependencyCandidate const& dc1,
    ...
    if (dc1.error_.GetMean() < dc2.error_.GetMean())
        ...
        if (dc1.vertical_.GetArity() < dc2.vertical_.GetArity())
            ...
            for (std::size_t a = dc1_cols.find_first(), b = dc2_cols.find_first();
                 a < dc1_cols.size(); a = dc1_cols.find_next(a), b = dc2_cols.find_next(b))
                if (a != b) return (a < b);
        }
    }
    return false;
