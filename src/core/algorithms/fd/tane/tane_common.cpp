...
void TaneCommon::ComputeDependencies(model::LatticeLevel* level) {
    ...
    for (auto& [key_map, xa_vertex] : level->GetVertices()) {
        ...
        if (xa_vertex->GetPositionListIndex() == nullptr) {
            auto parent_pli_1 = xa_vertex->GetParents()[0]->GetPositionListIndexWithSingletones();
            auto parent_pli_2 = xa_vertex->GetParents()[1]->GetPositionListIndexWithSingletones();
            xa_vertex->AcquirePLIWithSingletones(parent_pli_1->Intersect(parent_pli_2));
        }

        dynamic_bitset<> xa_indices = xa.GetColumnIndices();
        dynamic_bitset<> a_candidates = xa_vertex->GetRhsCandidates();
        auto xa_pli = xa_vertex->GetPositionListIndexWithSingletones();
        for (auto const& x_vertex : xa_vertex->GetParents()) {
            ...
            auto x_pli = x_vertex->GetPositionListIndexWithSingletones();
...
unsigned long long TaneCommon::ExecuteInternal() {
    ...
    for (auto& column : schema->GetColumns()) {
        ...
        vertex->SetPLIWithSingletones(column_data.GetPLWSIndex());
