...
void Aid::InvertNegativeCover() {
    ...
    for (size_t rhs = 0; rhs < number_of_attributes_; ++rhs) {
        auto report_fd_lhs = MakeReorderingLhsMaskAdder(storage.emplace_back(), attr_indices);
        if (constant_columns_[rhs]) {
            report_fd_lhs(boost::dynamic_bitset(number_of_attributes_));
            continue;
        }
        ...
        pos_cover_tree.ForEach(report_fd_lhs);
    }

    // This part would be done by the algorithm harness later.
    util::UndoPermutation(
            storage.size(), [&](model::Index i) { return attr_indices[i]; },
            [&](model::Index i, model::Index j) {
                std::swap(attr_indices[i], attr_indices[j]);
