...
private:
    ...
    public:
        ...
        model::FunctionalDependency operator*() const {
            ...
            boost::dynamic_bitset<> const& lhs_mask = *rhs_attr_fds_it_;
            lhs_attrs.reserve(lhs_mask.count());
            util::ForEachIndex(lhs_mask, [&](model::Index i) {
                lhs_attrs.emplace_back(storage_->table_header_.column_names[i], i);
            });
            return {storage_->table_header_.table_name,
                    std::move(lhs_attrs),
                    {{storage_->table_header_.column_names[rhs_attr_id_], rhs_attr_id_}}};
        }
...
public:
    LhsMaskFdView(model::TableHeader table_header, Storage lhs_masks)
        : table_header_(std::move(table_header)), lhs_masks_(std::move(lhs_masks)) {}
