...
Sampler::Traits::ClusterComparator::ClusterComparator(
    CompressedRecords const* sort_keys,
    ...
    if (traits.relation_->GetColumnData(comparison_column_1_).GetPositionListIndex().GetClustersNum() <
        traits.relation_->GetColumnData(comparison_column_2_).GetPositionListIndex().GetClustersNum()) {
...
void Sampler::Traits::Initialize(hy::Sampler<Traits>& sampler) {
    for (size_t bit = 0; bit < relation_->GetNumColumns(); ++bit) {
        ...
        sampler.plis_->emplace_back(std::make_shared<Traits::PLI>(clusters_iterator.begin(), clusters_iterator.end()));
    }
}
...
Sampler::Sampler(std::shared_ptr<DynamicRelationData> relation, size_t first_insert_batch_id,
    ...
    : sampler_(std::make_unique<std::vector<Traits::PLIPtr>>(),
               relation->GetCompressedRecordsPtr(),
               DynFDConfig::kSamplingEfficiencyThreshold,
               pool,
