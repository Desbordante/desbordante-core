...
public:
    ...
    explicit FDTreeElement(std::size_t max_attribute_number);
    ...
    [[nodiscard]] bool CheckFd(std::size_t index) const;

    [[nodiscard]] FDTreeElement* GetChild(std::size_t index) const;

    void AddFunctionalDependency(model::Bitset<kMaxAttrNum> const& lhs, std::size_t attr_num);

    // Searching for generalization of functional dependency in cover-trees.
    bool GetGeneralizationAndDelete(model::Bitset<kMaxAttrNum> const& lhs, std::size_t attr_num,
                                    std::size_t current_attr, model::Bitset<kMaxAttrNum>& spec_lhs);

    [[nodiscard]] bool ContainsGeneralization(model::Bitset<kMaxAttrNum> const& lhs,
                                              std::size_t attr_num, std::size_t current_attr) const;
...
private:
    ...
    std::size_t max_attribute_number_;
    model::Bitset<kMaxAttrNum> is_fd_;

    void AddRhsAttribute(std::size_t index);

    [[nodiscard]] model::Bitset<kMaxAttrNum> const& GetRhsAttributes() const;

    void MarkAsLast(std::size_t index);

    // Checking whether node is a leaf or not.
    [[nodiscard]] bool IsFinalNode(std::size_t attr_num) const;
