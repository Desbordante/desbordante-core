...
// Number of bits that can be stored in one `char`
static constexpr size_t kCharBit = 8;
...
concept HasExtesnsions = requires(Bitset bs) {
    bs._Find_first();
    bs._Find_next(0);
};
...
template <size_t S>
    requires HasExtesnsions<std::bitset<S>>
...
template <size_t NumBits>
struct hash<model::bitset_impl::BitsetImpl<NumBits>> {
