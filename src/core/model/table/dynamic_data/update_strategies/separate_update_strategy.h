...
private:
    ...
    void ProcessUpdate(IDynamicTableData* table) {
        ...
        while (update_data_->HasNextRow()) {
            ...
            if (!table->RowExists(row_id)) {
                throw config::ConfigurationError("Attempt to update a nonexistent row");
            }
    ...
    void ProcessDelete(IDynamicTableData* table) {
        std::ranges::for_each(delete_data_, [&table](size_t index) {
            if (!table->RowExists(index)) {
                throw config::ConfigurationError("Attempt to delete a nonexistent row");
            }
