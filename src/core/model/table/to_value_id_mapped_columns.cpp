...
std::vector<std::vector<int>> ToValueIdMappedColumns(model::IDatasetStream& data_stream) {
    ...
    auto value_id_mapped_columns = std::vector<std::vector<int>>(num_columns);
    ...
    while (data_stream.HasNextRow()) {
        ...
        for (model::Index column_index = 0; column_index != num_columns; ++column_index) {
            ...
            value_id_mapped_columns[column_index].push_back(it->second);
        }
    }

    return value_id_mapped_columns;
}
}  // namespace model
