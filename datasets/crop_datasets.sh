...
# Neighbors 50k could be used here, but it's stored in a separate archive in desbordante-data
# and therefore not cloned by no-LFS version of `pull_datasets` (which is used, for example, in CI)
cat neighbors100k.csv neighbors100k.csv | head -n 120001 > neighbors120k.csv
