...
# Mushroom with 2 additional attributes
...
head -n1501 cfd_data/mushroom.csv | awk 'function rnd() { return int(rand() * 100) }; \
    {if (FNR == 1) { print "rand1,rand2," $0 } else { print rnd() "," rnd() "," $0 } }' \
	>'mushroom+2attr1500.csv'
# Mushroom with 3 additional attributes
head -n1301 mushroom+2attr1500.csv | \
	awk '{if (FNR == 1) { print "rand0," $0 } else { print int(rand() * 100) "," $0 } }' \
    >'mushroom+3attr1300.csv'
